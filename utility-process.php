...
if (!$ut->valid_csrf()){
  ...
  //lets see if the user has enough balance to make a utility payment
if($auth_user[0]->balance < $_POST['amount']){
    $error = ['Not Enough Funds for this Transfer!'];
    $ut->redirect_with_errors('utility.php', $error);
  // exit();
}
else{
    //there's enough funds so let's initiate the transfer into the respective utility account
    ...
    $details = "Utility Payment " . $recepient[0]->first_name;
...
//lets make a database connection to run the above queries
 ...
 //runs the query for the transactional record
  if($db->query($transaction_sql)){
    ...
    $msg = "Successful Utility Payment " . $recepient[0]->first_name;;
  $ut->redirect_with_success('utility.php', $msg);
